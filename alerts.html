...
window.contactUser = function(phoneNumber, userName) {
    ...
    if (confirm(`Call ${userName} at ${phoneNumber}?`)) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Contact action
    document.getElementById('contactAction').addEventListener('click', function() {
        if (currentContactInfo && currentContactInfo.phone) {
            contactUser(currentContactInfo.phone, currentContactInfo.name);
        }
    });
